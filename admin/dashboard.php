                    ...
                    <div class="card">
                        <div class="card-header">
                            <h4 class="card-title">
                                ...
                                $month = (int)date("m");
                                $year = (int)date("Y");
                                $day_name = $db_handle->runQuery("SELECT sum(product_total_price) as price FROM `billing_details` as b,`invoice_details` as i WHERE i.billing_id=b.id and b.approve=2 and MONTH(b.updated_at)=$month and YEAR(b.updated_at)=$year");
            ...
            let barChart1 = function () {
                if (jQuery('#barChart_1').length > 0) {
                    ...
                    let barChartData1 = {
                        ...
                        labels: [<?php
                            $month_name = $db_handle->runQuery("SELECT MONTH(b.updated_at) as month_name, sum(product_total_price) as price FROM `billing_details` as b,`invoice_details` as i WHERE i.billing_id=b.id and b.approve=2 group by MONTH(b.updated_at), YEAR(b.updated_at) order by YEAR(b.updated_at),MONTH(b.updated_at),DAY(b.updated_at);");
                            $row_count = $db_handle->numRows("SELECT MONTH(b.updated_at) as month_name, sum(product_total_price) as price FROM `billing_details` as b,`invoice_details` as i WHERE i.billing_id=b.id and b.approve=2 group by MONTH(b.updated_at), YEAR(b.updated_at) order by YEAR(b.updated_at),MONTH(b.updated_at),DAY(b.updated_at);");

                            $date = '';
                            for ($i = $row_count-12; $i < $row_count; $i++) {
                        ...
                        datasets: [{
                            ...
                            data: [<?php
                                $month_name = $db_handle->runQuery("SELECT MONTH(b.updated_at) as month_name, sum(product_total_price) as price, sum(product_quantity) as qty,count(b.id) as person FROM `billing_details` as b,`invoice_details` as i WHERE i.billing_id=b.id and b.approve=2 group by MONTH(b.updated_at), YEAR(b.updated_at) order by YEAR(b.updated_at),MONTH(b.updated_at),DAY(b.updated_at);");
                                $row_count = $db_handle->numRows("SELECT MONTH(b.updated_at) as month_name, sum(product_total_price) as price FROM `billing_details` as b,`invoice_details` as i WHERE i.billing_id=b.id and b.approve=2 group by MONTH(b.updated_at), YEAR(b.updated_at) order by YEAR(b.updated_at),MONTH(b.updated_at),DAY(b.updated_at)");
                ...
                if (jQuery('#barChart_3').length > 0) {
                    ...
                    let barChartData = {
                        ...
                        labels: [<?php
                            $month = (int)date("m");
                            $year = (int)date("Y");
                            $day_name = $db_handle->runQuery("SELECT DAY(b.updated_at) as day_name, sum(product_total_price) as price, sum(product_quantity) as qty,count(b.id) as person FROM `billing_details` as b,`invoice_details` as i WHERE i.billing_id=b.id and b.approve=2 and MONTH(b.updated_at)=$month and YEAR(b.updated_at)=$year group by DAY(b.updated_at);");
                            $row_count = $db_handle->numRows("SELECT DAY(b.updated_at) as day_name, sum(product_total_price) as price, sum(product_quantity) as qty,count(b.id) as person FROM `billing_details` as b,`invoice_details` as i WHERE i.billing_id=b.id and b.approve=2 and MONTH(b.updated_at)=$month and YEAR(b.updated_at)=$year group by DAY(b.updated_at);");
                            $date = '';
